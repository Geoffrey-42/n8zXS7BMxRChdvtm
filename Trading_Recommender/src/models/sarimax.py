...
def sarimax_forecast(args):
    ...
    seasonal_order = model_args['seasonal_order']
    ...
    ## 2) Fit the SARIMAX to the historical data
    if autosearch_arima:
        from pmdarima.arima import auto_arima
        auto_search = auto_arima(to_fit, seasonal=True, m=10, trace=True)
        arima_order = auto_search.order
        seasonal_order = auto_search.seasonal_order
    model = SARIMAX(to_fit,
                    order = arima_order,
